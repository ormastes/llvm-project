//===-- TISCDisassembler.cpp - Disassembler for TISC ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the TISCDisassembler class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "tisc-disassembler"

typedef MCDisassembler::DecodeStatus DecodeStatus;

namespace {
class TISCDisassembler : public MCDisassembler {
...
public:
  TISCDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx)
...
static MCDisassembler *createTISCDisassembler(const Target &T,
                                                const MCSubtargetInfo &STI,
                                                MCContext &Ctx) {
  return new TISCDisassembler(STI, Ctx);
}

extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeTISCDisassembler() {
  TargetRegistry::RegisterMCDisassembler(getTheTISCTarget(),
                                         createTISCDisassembler);
}

static const unsigned GR8DecoderTable[] = {
  TISC::PCB,  TISC::SPB,  TISC::SRB,  TISC::CGB,
  TISC::R4B,  TISC::R5B,  TISC::R6B,  TISC::R7B,
  TISC::R8B,  TISC::R9B,  TISC::R10B, TISC::R11B,
  TISC::R12B, TISC::R13B, TISC::R14B, TISC::R15B
};
...
static const unsigned GR16DecoderTable[] = {
  TISC::PC,  TISC::SP,  TISC::SR,  TISC::CG,
...
#include "TISCGenDisassemblerTables.inc"
...
DecodeStatus TISCDisassembler::getInstructionI(MCInst &MI, uint64_t &Size,
...
DecodeStatus TISCDisassembler::getInstructionII(MCInst &MI, uint64_t &Size,
                                                  ArrayRef<uint8_t> Bytes,
                                                  uint64_t Address,
                                                  raw_ostream &CStream) const {
  uint64_t Insn = support::endian::read16le(Bytes.data());
...
static TISCCC::CondCodes getCondCode(unsigned Cond) {
  switch (Cond) {
  case 0: return TISCCC::COND_NE;
  case 1: return TISCCC::COND_E;
  case 2: return TISCCC::COND_LO;
  case 3: return TISCCC::COND_HS;
  case 4: return TISCCC::COND_N;
  case 5: return TISCCC::COND_GE;
  case 6: return TISCCC::COND_L;
  case 7: return TISCCC::COND_NONE;
  default:
    llvm_unreachable("Cond out of range");
  }
}

DecodeStatus TISCDisassembler::getInstructionCJ(MCInst &MI, uint64_t &Size,
                                                  ArrayRef<uint8_t> Bytes,
                                                  uint64_t Address,
                                                  raw_ostream &CStream) const {
  uint64_t Insn = support::endian::read16le(Bytes.data());
  ...
  if (Cond == 7)
    MI.setOpcode(TISC::JMP);
  else {
    MI.setOpcode(TISC::JCC);
...
DecodeStatus TISCDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
                                                ArrayRef<uint8_t> Bytes,
                                                uint64_t Address,
                                                raw_ostream &CStream) const {
  if (Bytes.size() < 2) {
