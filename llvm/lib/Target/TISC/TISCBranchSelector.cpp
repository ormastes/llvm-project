//===-- TISCBranchSelector.cpp - Emit long conditional branches ---------===//
//
...
#define DEBUG_TYPE "tisc-branch-select"

static cl::opt<bool>
    BranchSelectEnabled("tisc-branch-select", cl::Hidden, cl::init(true),
...
namespace {
class TISCBSel : public MachineFunctionPass {
...
public:
  static char ID;
  TISCBSel() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override { return "TISC Branch Selector"; }
};
char TISCBSel::ID = 0;
}
...
unsigned TISCBSel::measureFunction(OffsetVector &BlockOffsets,
...
bool TISCBSel::expandBranches(OffsetVector &BlockOffsets) {
  ...
  for (auto MBB = MF->begin(), E = MF->end(); MBB != E; ++MBB) {
    ...
    for (auto MI = MBB->begin(), EE = MBB->end(); MI != EE; ++MI) {
      ...
      if (MI->getOpcode() != TISC::JCC && MI->getOpcode() != TISC::JMP) {
        continue;
      }
      ...
      if (MI->getOpcode() == TISC::JCC && std::next(MI) != EE) {
      ...
      MI = BuildMI(*MBB, MI, dl, TII->get(TISC::Bi)).addMBB(DestBB);
      InstrSizeDiff += TII->getInstSizeInBytes(*MI);
...
bool TISCBSel::runOnMachineFunction(MachineFunction &mf) {
  MF = &mf;
  TII = static_cast<const TISCInstrInfo *>(MF->getSubtarget().getInstrInfo());
...
FunctionPass *llvm::createTISCBranchSelectionPass() {
  return new TISCBSel();
}
